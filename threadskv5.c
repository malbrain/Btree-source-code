// btree version threadskv5 sched_yield version
...
{
#ifdef unix
	return __sync_fetch_and_add (bt->mgr->latchmgr->dups, 1) + 1;
#else
	return _InterlockedIncrement64(bt->mgr->latchmgr->dups, 1);
#endif
}
...
int blk;
	...
	if( set->page_no = bt_getid(bt->mgr->latchmgr->chain) ) {
		...
		set->page_no = bt_getid(bt->mgr->latchmgr->alloc->right);
		bt_putid(bt->mgr->latchmgr->alloc->right, set->page_no+1);
#ifdef unix
		// if writing first page of pool block, set file length thru last page

		if( (set->page_no & bt->mgr->poolmask) == 0 )
			ftruncate (bt->mgr->idx, (set->page_no + bt->mgr->poolmask + 1) << bt->mgr->page_bits);
#endif
		if( set->pool = bt_pinpool (bt, set->page_no) )
			set->page = bt_page (bt, set->pool, set->page_no);
		else
...
BtPageSet right2[1];
	...
	if( !right_page_no ) {
	  bt_putid (bt->mgr->latchmgr->alloc->left, left_page_no);
	  return 0;
	}

	//	link right page to left page

	right2->latch = bt_pinlatch (bt, right_page_no);
	bt_lockpage (BtLockWrite, right2->latch);

	if( right2->pool = bt_pinpool (bt, right_page_no) )
		right2->page = bt_page (bt, right2->pool, right_page_no);
	else
		return bt->err;
...
BtVal *val;
	...
	if( bt_newpage(bt, left) )
		return bt->err;

	memcpy(left->page, root->page, bt->mgr->page_size);
	bt_unpinpool (left->pool);
...
{
uid page_no = bt_getid (bt->mgr->latchmgr->alloc->left);
BtPageSet set[1];
uint slot;
...
uid left;
	...
	if( set->pool = bt_pinpool (bt, left) )
		set->page = bt_page (bt, set->pool, left);
	else
		return 0;

	set->latch = bt_pinlatch (bt, left);
    bt_lockpage(BtLockRead, set->latch);
